...
class CategoryRepository with AbstractApiRepositories<FoodCategory, FoodCategoryDto> {
  ...
  final response = await http.get(Uri.parse('$baseUrl:$port/api/$version/categories'));
  if (response.statusCode == 200) {
    final Map<String, dynamic> body = json.decode(response.body);
    
    final List<dynamic> items = body['data'];
   
    return items.map((item) => createModel().fromJson(item) as FoodCategory).toList();
  } else {
